...
namespace WorldMapStrategyKit
{
	...
	{
		private int tickerLine;
		private WMSK_Ticker _ticker;
		...
		private void OnEnable()
		{
			...
			for (var k = 0; k < tickerNames.Length; k++)
		...
		public override void OnInspectorGUI()
		{
			...
			GUILayout.Label(
				new GUIContent("Overlay Mode",
					"When enabled, tickers will appear in front of Camera on top of everything."),
				GUILayout.Width(120));
			var oldOverlay = _ticker.overlayMode;
			...
			var oldb = tickers[tickerLine].visible;
			tickers[tickerLine].visible =
				EditorGUILayout.Toggle(tickers[tickerLine].visible, GUILayout.Width(20));
			...
			tickers[tickerLine].scrollSpeed =
				EditorGUILayout.Slider(tickers[tickerLine].scrollSpeed, -0.5f, 0.5f);
			...
			var oldf = tickers[tickerLine].verticalOffset;
			tickers[tickerLine].verticalOffset =
				EditorGUILayout.Slider(tickers[tickerLine].verticalOffset, -0.5f, 0.5f);
			...
			tickers[tickerLine].verticalSize =
				EditorGUILayout.Slider(tickers[tickerLine].verticalSize, 0.01f, 1.0f);
			...
			EditorGUILayout.BeginHorizontal();
			var autoHideContent =
				new GUIContent("   Auto Hide", "Ticker will automatically hide if it's empty");
			oldb = tickers[tickerLine].autoHide;
			GUILayout.Label(autoHideContent, GUILayout.Width(120));
			tickers[tickerLine].autoHide =
				EditorGUILayout.Toggle(tickers[tickerLine].autoHide, GUILayout.Width(20));
			...
			var oldc = tickers[tickerLine].backgroundColor;
			tickers[tickerLine].backgroundColor =
				EditorGUILayout.ColorField(tickers[tickerLine].backgroundColor, GUILayout.Width(50));
			...
			sampleTicker.tickerLine =
				EditorGUILayout.Popup(sampleTicker.tickerLine, tickerNames, GUILayout.Width(80));
			EditorGUILayout.EndHorizontal();
			...
			sampleTicker.horizontalOffset =
				EditorGUILayout.Slider(sampleTicker.horizontalOffset, -0.5f, 0.5f);
			...
			var prevFont = sampleTicker.font;
			...
			sampleTicker.textColor =
				EditorGUILayout.ColorField(sampleTicker.textColor, GUILayout.Width(50));
			EditorGUILayout.EndHorizontal();
			...
			if (sampleTicker.drawTextShadow)
			{
				...
				sampleTicker.shadowColor =
					EditorGUILayout.ColorField(sampleTicker.shadowColor, GUILayout.Width(50));
			}
			EditorGUILayout.EndHorizontal();
			...
			sampleTicker.blinkRepetitions =
				EditorGUILayout.IntSlider(sampleTicker.blinkRepetitions, 0, 50);
			EditorGUILayout.EndHorizontal();
				...
				if (sampleTicker.duration == 0)
					sampleTicker =
						(TickerText)sampleTicker
							.Clone(); // maintain properties but allow to create a new/different ticker without affecting previous one
			}
