...
namespace WorldMapStrategyKit.MapGenerator.Geom
{
	...
	{
		private List<SweepEvent> elements;
		private bool sorted;
		private int pointer;
...
//			int count = pointer = elements.Count;
		...
		private int CompareSweepEvent(SweepEvent e1, SweepEvent e2)
		{
			if (e1.Equals(e2))
				return 0;
			...
			if (
				e1.p.y - e2.p.y >
				Point.PRECISION) // Different points, but same x coordinate. The event with lower y coordinate is processed first
				return -1;
			...
			var isAbove = e1.isAbove(e2.otherSE.p);
			return isAbove ? -1 : 1;
		}
