...
{
	internal class SweepEventSet
	{
		public List<SweepEvent> eventSet;

		public SweepEventSet() => eventSet = new List<SweepEvent>(1000);
		...
		private bool SegmentCompare(SweepEvent e1, SweepEvent e2)
		{
			...
			if (signedArea(e1.p, e1.otherSE.p, e2.p) != 0 ||
			    signedArea(e1.p, e1.otherSE.p, e2.otherSE.p) != 0)
			{
		...
		private bool CompareSweepEvent(SweepEvent e1, SweepEvent e2)
		{
			...
			if (
				e1.p !=
				e2.p) // Different points, but same x coordinate. The event with lower y coordinate is processed first
				return e1.p.y > e2.p.y;

			if (
				e1.isLeft !=
				e2.isLeft) // Same point, but one is a left endpoint and the other a right endpoint. The right endpoint is processed first
				return e1.isLeft;
